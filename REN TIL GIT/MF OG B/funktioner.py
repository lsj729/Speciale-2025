...
def plot_input_effect_on_C(
    ...
    model2,
    model3,
    inp="ra",                 # choose "ra", "Z", or "chi" (or any input key you have)
    output="C_hh",
    T=40,
    label1="baseline",
    ...
    title=None,
    scale="pct",              # "pct" = % of SS C, "level" = level units
    ls2="--",
    color1="navy",
    color2="firebrick",
    color3="darkgreen"
    ...
    """
    Plot the contribution of a chosen input (ra, Z, chi, ...) to aggregate consumption
    for two models:
        dC_from_inp(t) = J_{C,inp} @ IRF_inp

    Requirements:
      - model.jac_hh[(output, inp)] exists
      - model.IRF[inp] exists
      - model.ss.C_hh exists (if scale="pct")

    Returns
    -------
    d1, d2 : arrays length T
        The input contribution series for model1 and model2.
    """
    ...
    d3 = contrib(model3, inp, output, T, scale)
    ...
    plt.plot(t, d3, lw=2.6, color=color3, ls="-.", label=label3)
    plt.title(title)
    plt.xlabel("Quarters")
